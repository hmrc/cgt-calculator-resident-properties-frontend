...
class WorthWhenInheritedActionSpec extends UnitSpec with WithFakeApplication with FakeRequestHelper with CommonMocks with MockitoSugar with GainControllerBaseSpec {
  ...
  def setupTarget(getData: Option[WorthWhenInheritedModel]): GainController= {
    ...
    when(mockSessionCacheConnector.saveFormData[WorthWhenInheritedModel](ArgumentMatchers.any(), ArgumentMatchers.any())
      (ArgumentMatchers.any(), ArgumentMatchers.any()))
